...
export default function SearchUpdateAnimalForm() {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/animal/${animalName}`);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/animal/${animalData.id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="w-full mt-8">
      ...
      <form onSubmit={handleSearch} className="mb-6">
        <label htmlFor="animalName" className="block text-lg font-medium mb-2">Search Animal by Name:</label>
        <input
          type="text"
          id="animalName"
          value={animalName}
      ...
      {/* Animal Form */}
      <form onSubmit={mode === 'update' ? handleUpdate : handleCreate} className="w-full">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <input
            type="text"
            name="weight"
            value={mode === 'update'? animalData.weight
            : newAnimal.weight}
            onChange={mode === 'update' ? handleChange : handleNewAnimalChange}
            placeholder="Weight"
            className="p-2 border border-gray-300 rounded"
        ...
        <div className="flex space-x-4">
          <button
            type="submit"
            className={`px-4 py-2 rounded ${mode === 'update' ? 'bg-green-500 text-white' : 'bg-blue-500 text-white'}`}
          >
